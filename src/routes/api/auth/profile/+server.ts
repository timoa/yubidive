...
export async function PUT({ request, locals }: RequestEvent) {
    try {
        ...
        if (newPassword) {
            ...
            if (!userWithPassword) {
                logger.error('User not found during password verification', { userId: currentUser.id });
                return json({
            ...
            if (!validPassword) {
                logger.warn('Invalid current password during password change', { userId: currentUser.id });
                return json({
